...
class ReportGenerator:
    ...
    def _evidence_url(self, path: str | None) -> str | None:
        if not path:
            return None
    ...
    async def generate(
        ...
        risk_score, risk_level = EventEngine.calculate_risk(
            events
        )
        ...
        incident_timeline = [
            {
                ...
                "evidence_path": self._evidence_url(
                    e.evidence_path
                ),
            }
            for e in events
        ...
        report = {
            ...
            "risk_summary": {
                ...
                "disclaimer": (
                    ...
                    "not a military threat assessment model."
                ),
            },
            "evidence_gallery": [
                {
                    "event_id": e.id,
                    "path": self._evidence_url(
                        e.evidence_path
                    ),
                    "type": e.type,
                }
    ...
    def _build_final_summary(
        ...
        if critical == 0 and warning == 0:
            return (
                "No warning or critical security events were "
                "detected during the surveillance session."
            )

        breach = sum(
            1
            for e in events
            if e.type == "VIRTUAL_FENCE_BREACH"
        )

        loitering = sum(
            1
            for e in events
            if e.type == "LOITERING_DETECTED"
        )
        ...
        if critical:
            parts.append(
                f"{critical} critical security event(s)"
            )

        if warning:
            parts.append(
                f"{warning} warning event(s)"
            )
        ...
        if breach:
            detail.append(
                f"{breach} virtual fence breach(es)"
            )

        if loitering:
            detail.append(
                f"{loitering} loitering event(s)"
            )

        summary = (
            f"{' and '.join(parts)} were detected "
            "during the surveillance session"
        )

        if detail:
            summary += (
                f", including {' and '.join(detail)}"
            )
