...
1. **Add a `POST /api/v1/heartbeat` endpoint.** The edge side is now fully implemented (sending CPU, memory, temp, queue depth, active cameras to `HEARTBEAT_URL`). The dashboard should show device health prominently.

2. **Add device auto-registration.** When a new edge device sends its first alert, auto-create a `devices` row with `is_approved = false`. An admin approves it via the frontend. This avoids manual pre-provisioning for field deployments.

3. **Add alert severity classification.** Not all alerts are equal. A person crossing a virtual border at 3 AM is higher priority than a car detected at noon. Add a `severity` field (`info`, `warning`, `critical`) computed from:
   - Feature type (virtual_border crossing > general detection)
   - Time of day (nighttime = higher severity)
   - Watchlist match (face/plate match = critical)
   - Zone (intrusion zone = higher than general area)

4. **Add a snapshot endpoint.** Allow the frontend to request the latest frame from a specific camera on demand: `POST /api/v1/devices/:id/cameras/:cam/snapshot` → SSE pushes a one-time `capture` event to the edge → edge sends back a frame. Useful for verifying camera angles when configuring zones.

5. **Implement evidence deduplication.** With 5 cameras at 2 FPS processing, identical or near-identical scenes produce redundant evidence. Consider perceptual hashing (pHash) on evidence images to avoid storing duplicate frames within a short time window.

6. **Add offline operation mode documentation.** Border areas may lose connectivity for hours. The edge already queues alerts in a JSONL outbox. The server should handle burst-replay gracefully — accept queued alerts with past timestamps without triggering duplicate face/ANPR processing.

7. **Consider MQTT as a future alternative to SSE.** If the deployment grows beyond 100 edge devices or needs bidirectional real-time communication (e.g., snapshot requests, firmware updates), MQTT with a broker like Mosquitto would scale better. SSE is perfect for the current scale.
